...
HackathonEtaIC::HackathonEtaIC(const InputParameters & parameters) :
...
Real
HackathonEtaIC::value(const Point & p)
{
  Real delta_eta = std::cos(_q1(0)*p(0)-4)*std::cos(_q1(1)*p(1))
    + std::cos(_q2(0)*p(0))*std::cos(_q2(1)*p(1))
    + _h*std::pow( std::cos(_q3(0)*p(0) + _q3(1)*p(1))*std::cos(_q4(0)*p(0) -_q4(1)*p(1)) , 2.0 );

  return _epsilon*std::pow( delta_eta, 2.0 );
}
